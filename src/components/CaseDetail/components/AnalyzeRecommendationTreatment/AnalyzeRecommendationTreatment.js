...
class AnalyzeRecommendationTreatment extends Component {
    async componentDidMount() {
        if (this.props.analyzeRecommendationTreatment !== undefined) {
            await this.props.analyzeRecommendationTreatmentStore.setEditableData(this.props.analyzeRecommendationTreatment);
        }
    }
    ...
    render() {
        ...
        return (
            <div id='ANLTreatment-Case' className={cx('AnalyzeRecommendationTreatment')}>
                <Collapsible transitionTime={150} open={open} trigger="추천처방분석">
                    <Container fluid>
